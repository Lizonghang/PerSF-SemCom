...
class GeneticAlgorithmBase(SkoBase, metaclass=ABCMeta):
    ...
    def run(self, max_iter=None):
        ...
        for i in range(self.max_iter):
            ...
            Y_score = -self.Y_raw
            generation_best_index = Y_score.argmax()
            self.generation_best_X.append(self.X[generation_best_index, :])
            self.generation_best_Y.append(Y_score[generation_best_index])
            self.all_history_Y.append(Y_score)
            self.all_history_FitV.append(self.FitV)

            if self.early_stop:
                best.append(max(self.generation_best_Y))
                if len(best) >= self.early_stop:
                    if best.count(max(best)) == len(best):
                        break
                    else:
                        best.pop(0)
